...
class OfflineFolderView extends StatefulWidget {
  const OfflineFolderView({Key? key, required this.folderModel})
      : super(key: key);
...
class _OfflineFolderViewState extends State<OfflineFolderView> {
  ...
  fetchAllSongs() async {
    if (mounted) {
      final img = await HelperFunctions.getLocalAlbumArtworkImage(
          widget.folderModel.id);
      if (img != null) {
        albumArt = DecorationImage(image: img, fit: BoxFit.fill);
      } else {
        albumArt = null;
      }
      fetchedSongs = await audioQuery.queryAudiosFrom(
          AudiosFromType.ALBUM_ID, widget.folderModel.id,
          sortType: null,
          orderType: OrderType.ASC_OR_SMALLER,
          ignoreCase: true);
      for (SongModel song in fetchedSongs) {
        isAddedToQueue =
            HelperFunctions.checkIfAddedInQueue(song.id.toString());
        Uint8List? bytes =
            await HelperFunctions.getLocalSongArtworkUri(song.id);
  ...
  updateFirstSongIndex() async {
    firstSongIndexInQueue =
        await HelperFunctions.getQueueIndexBySongId(songs[0].id);
  }
  ...
  bool doesBelong() {
    try {
      ...
      if (kDebugMode) {
        print(
            "offline model : current index : ${mainAudioPlayer.currentIndex!} , first song index : $firstSongIndexInQueue , queue len :${AppRouter.queue.length}");
      }
      if (firstSongIndexInQueue != -1 &&
          mainAudioPlayer.currentIndex! >= firstSongIndexInQueue &&
          songs[mainAudioPlayer.currentIndex! - firstSongIndexInQueue].id ==
              mainAudioPlayer
                  .sequence![mainAudioPlayer.currentIndex!].tag.extras["id"]) {
        //does belong
        return true;
      }
      //does not belong
  ...
  Widget playPauseButton(
      AudioPlayer player, PlayerState playerState, double iconSize) {
    ...
    if (processingState == ProcessingState.loading ||
      ...
      return SpinKitRipple(
        ...
        (!doesBelong() && processingState == ProcessingState.completed)) {
      //play button
      ...
      return GestureDetector(
        onTap: () => player.seek(Duration.zero, index: firstSongIndexInQueue),
        child: Icon(
          Icons.replay,
          color: Colors.white,
          size: iconSize,
                      ...
                      Container(
                        decoration: BoxDecoration(
                            color: Colors.accents
                                .elementAt(widget.folderModel.id %
